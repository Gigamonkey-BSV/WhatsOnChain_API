...
namespace WhatsOnChain {
    ...
    Bitcoin::TxID inline read_TxID (const JSON &j) {
        return Bitcoin::TxID {std::string {"0x"} + std::string (j)};
    }

    awaitable<bool> transactions::broadcast (const bytes &tx) {
    ...
    awaitable<list<UTXO>> addresses::get_unspent (const Bitcoin::address &addr) {
    ...
    awaitable<list<UTXO>> scripts::get_unspent (const digest256 &script_hash) {
    ...
    awaitable<list<Bitcoin::TxID>> addresses::get_history (const Bitcoin::address& addr) {
    ...
    awaitable<list<Bitcoin::TxID>> scripts::get_history (const digest256 &script_hash) {
    ...
    awaitable<bytes> transactions::get_raw (const Bitcoin::TxID &txid) {
    ...
    awaitable<JSON> transactions::tx_data (const Bitcoin::TxID &txid) {
    ...
    awaitable<maybe<merkle_proof>> transactions::get_merkle_proof (const Bitcoin::TxID &txid) {
        using result = maybe<merkle_proof>;
    ...
    awaitable<header> blocks::get_header_by_hash (const digest256 &hash) {
    ...
    awaitable<header> blocks::get_header_by_height (const N &n) {
